...
if calc_and_plot["case_A_1"]:
	...
	simulation = Simulation(model="Steady", t_max=5, dt=0.1)
...
if calc_and_plot["case_A_2"]:
	...
	omega = 0.3/(2*np.pi*rotor.R)*flow.V0
	simulation = Simulation(model="Steady", t_max=1/omega, dt=0.6)
	change_param = "CT_steady"
	change_type = "sine"
	change_values = (0.5, 0.5, omega)
	results = calculate_case(rotor, flow, airfoil, simulation, change_param, change_type, change_values)
	plot_case_results(results, "../data/results", change_param, change_type, change_values)

if calc_and_plot["case_B_1"]:
	print("Working on case B.1")
	simulation = Simulation(model="Steady", t_max=5, dt=0.05)
	change_param = "U_inf"
	change_type = "step"
	change_values = (1, 1.5)
	results = calculate_case(rotor, flow, airfoil, simulation, change_param, change_type, change_values)
...
if calc_and_plot["case_B_1"]:
	...
	omega = 0.3/(2*np.pi*rotor.R)*flow.V0
	simulation = Simulation(model="Steady", t_max=1/omega, dt=0.7)
	change_param = "CT_steady"
	change_type = "sine"
	change_values = (1, 0.5, omega)
	results = calculate_case(rotor, flow, airfoil, simulation, change_param, change_type, change_values)
	plot_case_results(results, "../data/results", change_param, change_type, change_values)
